...
It is correct as chi-square here. The difference is you have a "one-way" here vs. comparing two categorical variables (e.g. species and watershed). I made some notes in your exploratory analysis file about how you could do this as a 2-way which might be more interesting.
...
Maybe a colorful bar plot? Just a nice looking bar plot would be fine.
...
As you noted, you are asking about differnce in group means, but you don't have any replication per watershed (see my notes in Exploratory Analysis) so effectively you don't have group means.
...
NaN means "not a number" - you are getting this error message because you don't have any replicates. What you need for A2 is a data frame with these columns:
Watershed
Transect
Weight

That way, R can use the different weights from each transect per watershed to get a mean for each watershed.

But are these weight data from 2 species? It doesn't make sense to me to combine a big and a small species - it will possibly wash out any effect of watershed. 
...
Good - but see how to fix the analysis.
...
Notice that you are unable to plot boxplots or error bars - because you have none of the variation in your dataset. Again, you need to use the different transects as replicates for each shannon index so that you can compare them. 
3. guess expected relationships (hypothesis)
...
Good, but it won't work without replication.
